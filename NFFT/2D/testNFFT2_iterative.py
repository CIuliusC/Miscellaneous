...
timingGPU_BLAS  = np.zeros((1, maxExp - minExp), dtype = np.float64)
speedUp         = np.zeros((1, maxExp - minExp), dtype = np.float64)
speedUpBLAS     = np.zeros((1, maxExp - minExp), dtype = np.float64)
errNorms        = np.zeros((1, maxExp - minExp), dtype = np.float64)
errNormsBLAS    = np.zeros((1, maxExp - minExp), dtype = np.float64)
for k in range(minExp, maxExp):
    ...
    d_result_BLAS   = gpuarray.zeros((N, M), dtype = np.complex128)
    ...
    lib.NDFT2_2D_GPU.argtypes = [POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double), POINTER(c_double), 
                                 c_int, c_int, c_int, c_int]
    time1 = time()
    lib.NDFT2_2D_GPU(ctypes.cast(d_x.ptr, POINTER(c_double)), 
                     ctypes.cast(d_y.ptr, POINTER(c_double)), 
                     ctypes.cast(d_u.ptr, POINTER(c_double)), 
                     ctypes.cast(d_v.ptr, POINTER(c_double)), 
                     ctypes.cast(d_data.ptr, POINTER(c_double)),
                     ctypes.cast(d_result_BLAS.ptr, POINTER(c_double)), 
                     N, 
                     M, 
                     M_x * M_y,
                     N * M) 
    time2 = time()
    timingGPU_BLAS[0, k - minExp] = (time2 - time1)
    ...
    h_result_BLAS = d_result_BLAS.get()
    h_result_BLAS = np.transpose(h_result_BLAS)
    ...
    errNormsBLAS[0, k - minExp] = 100 * np.sqrt(np.sum(np.float_power(np.abs(h_result_BLAS - h_result_C), exponent)) / np.sum(np.float_power(np.abs(h_result_C), exponent)))

    speedUp[0, k - minExp] = timingCPU[0, k - minExp] / timingGPU[0, k - minExp]
    speedUpBLAS[0, k - minExp] = timingGPU_BLAS[0, k - minExp] / timingGPU[0, k - minExp]
