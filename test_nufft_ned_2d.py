...
__device__ double spectralWindowKB_NFFT2_2D_c_1_5_K_6_Opt(const double argBessel, const double alfa_prime) {
...
__global__ void interpolationKB_NFFT2_2D_c_1_5_K_6_Opt(const double2* __restrict__ data, double2* __restrict__ result, const double* __restrict__ x, const double* __restrict__ y, const double alfa_prime, const int N1, const int N2, int M)
{
	...
	if ((i < M) && (m < 13) && (n < 13)) {
		...
		phi_cap1 	= spectralWindowKB_NFFT2_2D_c_1_5_K_6_Opt(cc_diff1 - (double)(m - 6), alfa_prime);	
		
		//PP2 			= modulo((r_cc_points2 + (n - K) + N2 * cc / 2), cc * N2);
		PP2 			= modulo((r_cc_points2 + (double)(n - 6) + 0.75* N2), 3 * N2 / 2);

		phi_cap2 	= phi_cap1 * spectralWindowKB_NFFT2_2D_c_1_5_K_6_Opt(cc_diff2 - (double)(n - 6), alfa_prime);	
...
__device__ double spatialWindowKB_NFFT2_2D_c_2_K_6_device_Opt(const int i, const int N)
{
    ...
    double x_in           = 3. * (static_cast<double>(2. * pi_double) * (double)i) / ((2. * pi_double - pi_double / 2.) * (double)(2 * N));
...
__device__ double spatialWindowKB_NFFT2_2D_c_1_5_K_6_device_Opt(const int i, const int N)
{
    ...
    double x_in           = 2. * (static_cast<double>(2. * pi_double) * (double)i) / ((2. * pi_double - pi_double / 1.5) * (1.5 * N));
...
__global__ void Decimation_and_Scaling_c_2_K_6_Opt(const double2* __restrict__ data, double2* __restrict__ result, const int N1, const int N2)
{
    ...
    if((((i >= N1 / 2)  && (i < 3 * N1 / 2)) && ((j >= N2 / 2)  && (j < 3 * N2 / 2)))) {
		...
		double a = spatialWindowKB_NFFT2_2D_c_2_K_6_device_Opt(i - N1, N1) * spatialWindowKB_NFFT2_2D_c_2_K_6_device_Opt(j - N2, N2);
...
__global__ void Decimation_and_Scaling_c_1_5_K_6_Opt(const double2* __restrict__ data, double2* __restrict__ result, const int N1, const int N2)
{
    ...
    if((((i >= N1 / 4)  && (i < 5 * N1 / 4)) && ((j >= N2 / 4)  && (j < 5 * N2 / 4)))) {
		...
		double a = spatialWindowKB_NFFT2_2D_c_1_5_K_6_device_Opt(i - N1, N1) * spatialWindowKB_NFFT2_2D_c_1_5_K_6_device_Opt(j - N2, N2);
...
def NUFFT_NED_2D_CUDA_Opt(d_data, d_y, d_x, M1, M2, c, K):
  ...
  alfa_prime  			= (2 - 1 / c) * np.pi
	...
	# --- Interpolation
  #tic = time.perf_counter()
  d_u 							= gpuarray.zeros((2 * M1, 2 * M2), dtype = np.complex128)
  #toc = time.perf_counter()
  ...
  #tic = time.perf_counter()
  if ((K == 6)):
    fftshift_2D_double(d_u, np.int32(2 * M1), np.int32(2 * M2), block = (16, 16, 1), grid = (int(iDivUp(2 * M1, 16)), int(iDivUp(2 * M2, 16)), 1))

  plan    = fft.Plan((2 * M1, 2 * M2), np.complex128, np.complex128)
  fft.fft(d_u, d_u, plan)

  if ((K == 6)):
    fftshift_2D_double(d_u, np.int32(2 * M1), np.int32(2 * M2), block = (16, 16, 1), grid = (int(iDivUp(2 * M1, 16)), int(iDivUp(2 * M2, 16)), 1))
  #toc = time.perf_counter()
  #print('FFT optimized = {}s'.format(toc - tic))
  ...
  #tic = time.perf_counter()
  if ((c == 2) & (K == 6)):
	  Decimation_and_Scaling_c_2_K_6_Opt(d_u, d_result, np.int32(M1), np.int32(M2), block = (16, 16, 1), grid = (int(iDivUp(2 * M2, 16)), int(iDivUp(2 * M1, 16)), 1))
  if ((c == 1.5) & (K == 6)):
	  Decimation_and_Scaling_c_1_5_K_6_Opt(d_u, d_result, np.int32(M1), np.int32(M2), block = (16, 16, 1), grid = (int(iDivUp(2 * M2, 16)), int(iDivUp(2 * M1, 16)), 1))
  #cuda.Context.synchronize()
  #toc = time.perf_counter()
...
tic = time.perf_counter()
d_transf_CUDA_Opt               = NUFFT_NED_2D_CUDA_Opt(d_data, d_y, d_x, M, N, c, K)
toc = time.perf_counter()
...
plt.imshow(np.abs(transf_CUDA))
plt.colorbar()
plt.show()

print(100 * np.sqrt(np.sum(np.sum(np.abs(u - u_CUDA)**2)) / np.sum(np.sum(abs(u)**2))))
...
plt.imshow(np.angle(u_CUDA / u))
plt.colorbar()
plt.show()

print(u[60, 60] / u_CUDA[60, 60])

print(u[70, 70] / u_CUDA[70, 70])
